...
class TestFREDIngestor:
    ...
    def test_fetch_observations_with_data(self, ingestor):
        ...
        for obs in observations:
            obs_date = obs['date'] if isinstance(
                obs['date'], date) else datetime.strptime(obs['date'], '%Y-%m-%d').date()
    ...
    def test_fetch_observations_date_ordering(self, ingestor):
        ...
        assert dates == sorted(
            dates), "Observations should be chronologically ordered"
    ...
    def test_register_series_creates_metadata(self, ingestor):
        ...
        with get_db_session() as session:
            ...
            # Fetch real metadata
            metadata_list = ingestor_with_session.fetch_series_metadata([
                                                                        'DGS10'])
    ...
    def test_store_observations_inserts_data(self, ingestor):
        ...
        with get_db_session() as session:
            ...
            if observations:
                records_inserted = ingestor_with_session.store_observations(
                    series_id,
                    observations
                )
    ...
    def test_fred_observations_have_reasonable_values(self):
        ...
        with get_db_session() as session:
            ...
            assert len(invalid_values) == 0, \
                f"Found invalid interest rate values: {invalid_values}"
