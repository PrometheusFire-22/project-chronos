...
@click.option(
    '--status',
    default='Planned',
    help='Ingest series with this status'
)
@click.option(
    '--no-update',
    is_flag=True,
    help='Do not update catalog status after ingestion'
)
@click.option(
    '--series',
    multiple=True,
    help='Ingest specific series IDs (overrides --status)'
)
